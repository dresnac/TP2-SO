...
tPid libc_create_process ( main_function rip, uint64_t priority, char ** argv, uint64_t argc, uint64_t fds[] )
{
	return ( tPid ) sys_create_process ( rip, priority, argv, argc, fds );
}
int64_t libc_get_time ( LocalTime * time )
{
...
int64_t libc_kill ( tPid pid )
{
	return sys_kill ( pid );
}
int64_t libc_block ( tPid pid )
{
	return sys_block ( pid );
}
int64_t libc_nice ( tPid pid, uint64_t newPrio )
{
...
int64_t libc_unblock ( tPid pid )
{
	return sys_unblock ( pid );
}

tPid libc_wait ( tPid pid, int64_t * ret )
{
...
int8_t libc_get_status ( tPid pid )
{
	return sys_get_status ( pid );
}
int64_t libc_pipe_open ( int64_t id, tPipeMode mode )
{
	return sys_pipe_open ( id, mode );
}
int64_t libc_pipe_open_free ( tPipeMode mode )
{
...
int64_t libc_get_my_fds ( uint64_t fds[CANT_FDS] )
{
